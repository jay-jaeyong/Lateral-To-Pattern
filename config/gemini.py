"""
Gemini Connection & Generation Configuration
---------------------------------------------
Gemini API 연결·생성 파라미터 설정.
생성 파라미터 조정은 이 파일에서만 수행하세요.

모델 이름은 여기에 없습니다. chats.create(model=...)가 세션 생성 시점에
모델을 고정하므로 모델은 서비스 단위 설정입니다 (services/*/service.py).
