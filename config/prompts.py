        ...
        "prompt": (
            """{
                ...
                "task": [
                    ...
                    "신발의 모든 선, 글자, 구멍, 재봉선, 윤곽, 자재 무늬 등 특징을 **추가/삭제/왜곡없이** 완벽하게 똑같이 그려",
                    "1, 2, 3에 입력 순서대로 제공된 칸에 신발 1개만 채워",
        ...
        "prompt": (
            """{
                ...
                "task_rules": {
                    ...
                    "view_mapping": {
                        "lateral_view": "멀티뷰 1번 이미지는 왼쪽 Toe(아래)에서 Heel(위)까지 전체, 멀티뷰 3번 이미지는 오른쪽 Toe(아래)에서 Heel(위)까지 전체를 채우는 데 사용해",
                        "top_view": "1번과 2번으로 채우면 TopView의 아래 부위에 발등 부위가 끊어질텐데, 멀티뷰 2번 이미지를 연결 고리로 자연스럽게 연결해",
                ...
                "caution": [
                    ...
                    "vamp는 단일 연속 표면으로 한 장처럼 매끄럽게 이어져야 함"
                    "Upper만 그림, Bottom이 그려지면 실격"
                    "1번,2번, 3번의 컨텍스트가 이어지고 연결 부위는 절대 추측하지않고 원본 이미지대로 연결"
                ]
            }"""     
        ),
            ...
            """
                {
                    ...
                    "caution": [
                        ...
                        "입력 이미지에 없는 선/점선/스티치/패턴은 **절대 생성하지 말고**, 보이지 않으면 비워둬",
                        "가이드라인에 맞춰서 이전 패턴을 사실 그대로 무늬/실/패턴/부품 등 전부 가져오면서 자연스럽게 확장해줘"
                    ]
                }
            """,
